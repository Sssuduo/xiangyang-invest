...
> 但 worker 把 `[识别失败：…]` 占位符当作“有内容”，状态误标为 `asr_completed`，
> 导致界面显示「转写完成」而内容为空，用户感知为“识别失败”。

### 修复转写失败被掩盖

- `run_async_processing` 逐文件转写失败时：
  - **记录异常日志**（`logger.error(..., exc_info=True)`），此前异常被静默吞掉、根因丢失；
  - **持久化 `audio_files[].error`**（`set_audio_files`），此前漏写导致 `status` 永远停在初始 `pending`。
- **全部文件均失败（`total_ok == 0`）时，状态判为 `asr_failed`**（而非 `asr_completed`），
  并在 `audio_summary` 保留首个文件的真实错误原因；部分成功仍走 `asr_completed`。

### 影响范围

- 真正“转写失败”的录音现在会正确显示「识别失败」并保留错误信息，便于排查；
  不再被误显示成「转写完成」。
- 前端已具备 `asr_failed` 展示（V15.6 适配），无需改动。
