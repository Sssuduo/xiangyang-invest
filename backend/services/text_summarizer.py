...
任务: 在"分段原文"基础上做【公文规范化润色 + 结构化整理】。参照《党政机关公文格式》(GB/T 9704-2012) 的文稿规范, 产出一份结构清晰、书面规范、可直接引用的会议纪要整理稿。

【一、结构重构规则】
1. 自动提炼规范标题: 根据会议内容生成正式纪要标题, 格式如《关于X月X日XX项目融资座谈会议纪要》。
2. 公文前置导语: 开头规范表述时间、组织单位、参会主体、座谈核心议题, 删除口语化断句、零散短句。
3. 按业务逻辑组织章节: 将原文信息按主题归并为若干一级章节(如: 一、项目基础概况; 二、当前融资困难与资金缺口现状; 三、合作对接可行性及制约因素; 四、下一步实施路径; 五、要素保障事项; 六、政策申报与扶持资金争取; 七、其他同步洽谈事项), 章节数量与命名根据实际内容灵活调整, 不增删核心业务数据。
4. 层级序号严格公文规范: 一级标题「一、」; 二级「（一）」; 三级「1.」; 四级「（1）」, 原文圆点/横杠散记全部转化为标准层级序号。

【二、文字、数据、标点整改规则】
1. 口语清理: 删除"提到、希望、认为、努力后、卡点"等口语化表述, 替换为政府公务书面用语; 修正断句碎片化、分句残缺问题, 合并同类零散要点。
2. 标点统一: 中文标点使用全角, 数字、金额、百分比、计量单位用半角; 顿号统一为标准全角顿号, 年份使用标准〔〕六角括号。
3. 数据规整: 所有资金、面积、产能、工期、成本数据统一格式、集中归类, 不零散穿插段落; 金额统一规范表述(如3000万元, 不出现模糊表述)。
4. 人名职务规范: 原文人名、职务统一规范表述, 职务前置或后置统一格式, 不简写混乱。
5. 修正逻辑断层: 原文内容中途截断、语句不完整的, 根据上下文业务逻辑补全通顺书面表述, 不丢失原有信息。
6. 区分项目主体: 不同项目(如乳制品项目、可降解地膜项目)分块隔离, 避免内容混杂; 遗留协调事项单独成块。
...
- 严禁使用圆点(●)、横杠(-)作为正式层级符号, 一律使用公文序号(一、/（一）/1./（1）)。
- 输出必须为结构化 Markdown: 标题用 #, 章节用 ##, 条目用序号。

输出: 仅输出整理后的 Markdown 文本, 不加任何说明。"""

CLEAN_USER_PROMPT = """请对以下带发言标记的会议文本进行公文规范化润色和结构化整理:
