  ...
  <div class="admin-layout">
    ...
    <main class="admin-main">
      <div class="admin-content">
        ...
        <p class="page-desc">配置招商动态导入模板的字段列。勾选「导出」的字段将出现在下载的导入模板中，「必填」标记的字段在导入时会进行非空校验。</p>
              ...
              <div class="preview-table-wrapper" v-if="enabledFields.length > 0">
                <div class="preview-table">
                  <div class="preview-header-row">
                    <div v-for="f in enabledFields" :key="f.field_key" class="preview-header-cell">
                      {{ f.field_label }}
                      <span v-if="f.is_required" class="required-star">*</span>
                    </div>
                  </div>
                  <div class="preview-data-row">
                    <div v-for="f in enabledFields" :key="f.field_key" class="preview-data-cell">
                      {{ sampleData[f.field_key] || '' }}
                    </div>
                  </div>
                  <div class="preview-mark-row">
                    <div v-for="f in enabledFields" :key="f.field_key" :class="['preview-mark-cell', f.is_required ? 'mark-required' : 'mark-optional']">
                      {{ f.is_required ? '（必填）' : '（选填）' }}
                    </div>
                  </div>
                </div>
          ...
          <el-col :span="10">
            <div class="card preview-card">
              ...
              <ul class="import-guide">
                ...
                <li><strong>所属项目</strong>必须填数据库中已有的项目名称</li>
                <li>日期格式：YYYY-MM-DD HH:MM（如 2026-01-01 10:00）</li>
                <li>附件(URL) 为选填，多个URL用英文逗号分隔</li>
...
import { getActivityImportFields, updateActivityImportFields, downloadActivityImportTemplate } from '@/api/activity_import'
...
async function loadFields() {
  loading.value = true
  try {
    const res = await getActivityImportFields()
...
async function handleSave() {
  ...
  try {
    const data = fields.value.map((f, i) => ({
      ...
      is_enabled: f.is_enabled, is_required: f.is_required,
      sort_order: i + 1
    }))
    const res = await updateActivityImportFields(data)
...
async function handleDownload() {
  try { await downloadActivityImportTemplate(); ElMessage.success('模板下载成功') }
  catch (err) { ElMessage.error(err.message) }
}
</script>
