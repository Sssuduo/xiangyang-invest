  ...
  <div class="admin-layout">
    ...
    <main class="admin-main">
      <div class="admin-content">
        ...
        <p class="page-desc">配置企业诉求导入模板的字段列。勾选「导出」的字段将出现在下载的导入模板中，「必填」标记的字段在导入时会进行非空校验。</p>
          ...
          <el-col :span="10">
            <div class="card preview-card">
              ...
              <ul class="import-guide">
                ...
                <li><strong>项目ID</strong>为必填，填写数据库中的项目数字ID</li>
                <li><strong>所属项目</strong>必须填数据库中已有的项目名称</li>
                <li>可通过诉求导入页面的「下载导入模板」功能，选择具体项目后下载预填模板</li>
...
import { getDemandImportFields, updateDemandImportFields, downloadDemandImportTemplate } from '@/api/demand'
...
async function loadFields() {
  loading.value = true
  try {
    const res = await getDemandImportFields()
...
async function handleSave() {
  ...
  try {
    ...
    const res = await updateDemandImportFields(data)
...
async function handleDownload() {
  try { await downloadDemandImportTemplate(); ElMessage.success('模板下载成功') }
  catch (err) { ElMessage.error(err.message) }
}
</script>
