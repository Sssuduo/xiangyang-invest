  ...
  <div class="admin-layout">
    ...
    <main class="admin-main">
      <div class="admin-content">
        ...
        <el-table :data="demands" row-key="id" v-loading="loading" @selection-change="handleSelectionChange" empty-text="暂无诉求数据">
          ...
          <el-table-column label="诉求内容" min-width="260" show-overflow-tooltip>
            <template #default="{ row }">{{ truncate(row.demand_content, 60) }}</template>
          </el-table-column>
...
function handleImportCmd(cmd) {
  if (cmd === 'download-template') {
    downloadDemandImportTemplate().catch(err => ElMessage.error(err.message))
